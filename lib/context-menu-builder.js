...
class ContextMenuBuilder {
  ...
  showPopupMenu(contextInfo) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!menu) return;
      menu.popup(_electron.remote.getCurrentWindow(), { async: true });
    })();
  }
  ...
  buildMenuForTextInput(menuInfo) {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this3.processMenu(menu, menuInfo);
  ...
  addSpellingItems(menu, menuInfo) {
    ...
    return _asyncToGenerator(function* () {
      let target = _this4.getWebContents();
      ...
      if (process.platform === 'darwin') {
        let learnWord = new MenuItem({
          label: _this4.stringTable.addToDictionary(),
          click: (() => {
            var _ref2 = _asyncToGenerator(function* () {
              ...
              try {
                yield _this4.spellCheckHandler.addToDictionary(menuInfo.misspelledWord);
              } catch (e) {
  ...
  addSearchItems(menu, menuInfo) {
    ...
    if (process.platform === 'darwin') {
      ...
      let lookUpDefinition = new MenuItem({
        label: this.stringTable.lookUpDefinition({ word: (0, _utility.truncateString)(menuInfo.selectionText) }),
